...
useInfo()
{
    infoText = str;
    useInfo.fill(false);
    std::vector<std::u32string> vecStr = split(str, U',');
    for (long unsigned int i = 0; i < vecStr.size(); i++)
    {
        ...
        {
            //infoText.setStyle(static_cast<InfoText::Style>(std::stoul(keyVal.second, nullptr, 10)));
            useInfo[4] = true;
        }
        else if (keyVal.first == InfoText::Keys32[5])
        {
            //infoText.setFillColor(sf::Color(std::stoul(keyVal.second, nullptr, 10)));
            useInfo[5] = true;
        }
        else if (keyVal.first == InfoText::Keys32[6])
        {
            //infoText.setOutlineColor(sf::Color(std::stoul(keyVal.second, nullptr, 10)));
            useInfo[6] = true;
        }
        ...
        else if (keyVal.first == InfoText::Keys32[8])
        {
            //infoText.setThickness(std::stof(keyVal.second, nullptr));
            useInfo[8] = true;
        }
        else if (keyVal.first == InfoText::Keys32[9])
        {
            //infoText.setThickness(std::stof(keyVal.second, nullptr));
            useInfo[9] = true;
        }
...
NodeText::Type BlockInfoText::getType() const
{
    return Type::BLOCKINFO;
}

const sf::Font* BlockInfoText::getFont() const
{
    return infoText.getFont();
}

long unsigned int BlockInfoText::getCharSize() const
{
    return infoText.getCharSize();
}

const std::tuple<bool, float>& BlockInfoText::getLetterSpacing() const
{
    return infoText.getLetterSpacing();
}

float BlockInfoText::getLineSpacingFactor() const
{
    return infoText.getLineSpacingFactor();
}

InfoText::Style BlockInfoText::getStyle() const
{
    return infoText.getStyle();
}
...
sf::Color BlockInfoText::getBackColor() const
{
    return infoText.getBackColor();
}

const sf::Vector2<float> BlockInfoText::getSizeBack() const
{
    return infoText.getSizeBack();
}

float BlockInfoText::getThickness() const
{
    return infoText.getThickness();
}

bool BlockInfoText::isUsed(long unsigned int i) const
{
    return useInfo[i];
}
...
const InfoText& BlockInfoText::getInfoText() const
{
    return infoText;
}
...
{
    bool useful = false;
    for (InfoText::Info i = InfoText::Info::INFO_BEGIN; i < InfoText::Info::NB_INFO; i++)
    {
...
BlockInfoText& BlockInfoText::operator+=(const BlockInfoText& right)
{
    ...
    {
        infoText.setInfo<InfoText::Info::FONT>(right.getInfoText().getInfo<InfoText::Info::FONT>());
        setUsed(InfoText::Info::FONT, true);
    }
    if (right.isUsed(InfoText::Info::CHAR_SIZE))
    {
        infoText.setInfo<InfoText::Info::CHAR_SIZE>(right.getInfoText().getInfo<InfoText::Info::CHAR_SIZE>());
        setUsed(InfoText::Info::CHAR_SIZE, true);
    }
    if (right.isUsed(InfoText::Info::LETTER_SPACING))
    {
        infoText.setInfo<InfoText::Info::LETTER_SPACING>(right.getInfoText().getInfo<InfoText::Info::LETTER_SPACING>());
        setUsed(InfoText::Info::LETTER_SPACING, true);
    }
    if (right.isUsed(InfoText::Info::LINE_SPACING_FACTOR))
    {
        infoText.setInfo<InfoText::Info::LINE_SPACING_FACTOR>(right.getInfoText().getInfo<InfoText::Info::LINE_SPACING_FACTOR>());
        setUsed(InfoText::Info::LINE_SPACING_FACTOR, true);
    }
    if (right.isUsed(InfoText::Info::STYLE))
    {
        infoText.setInfo<InfoText::Info::STYLE>(right.getInfoText().getInfo<InfoText::Info::STYLE>());
        setUsed(InfoText::Info::STYLE, true);
    }
    if (right.isUsed(InfoText::Info::FILL_COLOR))
    {
        infoText.setInfo<InfoText::Info::FILL_COLOR>(right.getInfoText().getInfo<InfoText::Info::FILL_COLOR>());
        setUsed(InfoText::Info::FILL_COLOR, true);
    }
    if (right.isUsed(InfoText::Info::OUTLINE_COLOR))
    {
        infoText.setInfo<InfoText::Info::OUTLINE_COLOR>(right.getInfoText().getInfo<InfoText::Info::OUTLINE_COLOR>());
        setUsed(InfoText::Info::OUTLINE_COLOR, true);
    }
    if (right.isUsed(InfoText::Info::BACK_COLOR))
    {
        infoText.setInfo<InfoText::Info::BACK_COLOR>(right.getInfoText().getInfo<InfoText::Info::BACK_COLOR>());
        setUsed(InfoText::Info::BACK_COLOR, true);
    }
    if (right.isUsed(InfoText::Info::SIZE_BACK))
    {
        infoText.setInfo<InfoText::Info::SIZE_BACK>(right.getInfoText().getInfo<InfoText::Info::SIZE_BACK>());
        setUsed(InfoText::Info::SIZE_BACK, true);
    }
    if (right.isUsed(InfoText::Info::THICKNESS))
    {
        infoText.setInfo<InfoText::Info::THICKNESS>(right.getInfoText().getInfo<InfoText::Info::THICKNESS>());
